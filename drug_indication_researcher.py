...
Looks for drug indications in innovator presentations and other
investor/company sources via BigQuery + Gemini enrichment.
...
def enrich_with_gemini(rows, molecule_name):
    enriched = []
    total = len(rows)
    checkpoint_file = f"checkpoint_{molecule_name.lower().replace(' ', '_')}_researcher.json"
...
"""
        ...
        for ind in processed:
            enriched.append({
                ...
                "data_source":     "Innovator Research",
            })
...
# ── COMMON FORMAT HEADERS (same as label.py) ─────────────────────────────────
...
def write_excel(rows, molecule_name):
    ...
    c = ws["A1"]
    c.value     = f"Drug Indication Research  —  {molecule_name.title()}"
