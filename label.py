...
# ----------------------------
# GEMINI ENRICHMENT (MULTI-CONDITION + CHECKPOINT)
# ----------------------------
...
- If only one condition → return list with one item
...
def write_excel(rows, molecule_name):
    ...
    for r, row in enumerate(rows, start=2):
        ...
        ws.cell(r, 2, row.get("condition"))
        ws.cell(r, 3, row.get("trial_title"))
        ws.cell(r, 4, row.get("trial_id"))
        ws.cell(r, 5, row.get("phase"))
